...
#  Marker  5570164814e0 tid: 0 addrcount 10778
        ...
        if [ $haswarmup -eq 1 ];
        then
          addr1_start_count=`grep -A2  "Warmup-Start tid: 0" $efile | grep "global_addrcount" | awk '{print $NF}'`
        addr1_end_count=`grep -A2  "Sim-Start tid: 0" $efile | grep "global_addrcount" | awk '{print $NF}'`
