...
#  Marker  5570164814e0 tid: 0 addrcount 10778
        ...
        if [ $haswarmup -eq 1 ];
        then
          addr1_start_count=`grep -A2  "Warmup-Start global" $eventfile | grep "global_addrcount" | tail -1 |  awk '{print $NF}'`
        addr1_end_count=`grep -A2  "Sim-Start global" $eventfile | grep "global_addrcount" | tail -1 |  awk '{print $NF}'`
        start_rel_count=`echo $addr1_end_count - $addr1_start_count | bc`
        else
            start_rel_count="1"
        fi
        addr2_start_count=`grep -A4  "Sim-Start tid: 0" $eventfile | grep "global_addrcount" | tail -1 | awk '{print $NF}'`
        addr2_end_count=`grep -A4  "Sim-End tid: 0" $eventfile | grep "global_addrcount"  | tail -1 | awk '{print $NF}'`
        end_rel_count=`echo $addr2_end_count - $addr2_start_count | bc`
COMMAND_NATIVE="\$SNIPER_ROOT/run_sniper \$SNIPER_ARGS --trace_args=\"-control start:address:$startImage+$startOffset:count$startCount:global -control stop:address:$endImage+$endOffset:count$endCount:global\" -- $APPCOMMAND"
COMMAND_RPB="\$SNIPER_ROOT/run_sniper \$SNIPER_ARGS --trace_args=\"-control start:address:$startPC:count$start_rel_count:global -control stop:address:$endPC:count$end_rel_count:global\" --pinballs $rpb"
COMMAND_ELFIE="cd $elfiedir; \$SNIPER_ROOT/run_sniper \$SNIPER_ARGS --trace_args=\"-control start:address:$startPC:count$start_rel_count:global -control stop:address:$endPC:count$end_rel_count:global\" -- ./$elfiename"
