...
#  Marker  5570164814e0 tid: 0 addrcount 10778
        ...
        if [ $haswarmup -eq 1 ];
        then
          addr1_start_count=`grep -A2  "Warmup-Start tid: 0" $eventfile | grep "global_addrcount" | awk '{print $NF}'`
        addr1_end_count=`grep -A2  "Sim-Start tid: 0" $eventfile | grep "global_addrcount" | awk '{print $NF}'`
        ...
        cat ../PinPoints/gem5_example_elfie_conf.py.txt | awk -v elfie_path=$cmd -v startpc=$startPC -v startpccount=$start_rel_count -v endpc=$endPC -v endpccount=$end_rel_count '
