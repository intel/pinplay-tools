...
COMMAND="\$SDE_ROOT/sde64 -t64 \$SDE_ROOT/intel64/sde-global-event-icounter.so -thread_count $OMP_NUM_THREADS -prefix $ppdir/$rpbname -controller_log -controller_olog pcevents.controller.$pgm.$i.$rid.txt -watch_addr $startPC -watch_addr $endPC -xyzzy  -control start:address:$startPC:count$startCount:global -control stop:address:$endPC:count$endCount:global -replay -replay:addr_trans -replay:basename $wpb -xyzzy -replay:deadlock_timeout 0 -- $SDE_ROOT/intel64/nullapp"
